...
int main(int argc, char** argv) {
 ...
 init_prectr_128();
  ...
  if (world_rank == 0) {   
    MPI_PreCtr_Send(send_buf, 190, MPI_INT, 1, 0, MPI_COMM_WORLD);
    //MPI_Wait(&request, &status);
  } else if (world_rank == 1) {
    MPI_PreCtr_Recv(recv_buf, 190, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
    //MPI_Wait(&request, &status);
  ...
  if (world_rank == 0) {   
    MPI_PreCtr_Send(d_send_buf, 98, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD);
    //MPI_Wait(&request, &status);
  } else if (world_rank == 1) {
    MPI_PreCtr_Recv(d_recv_buf, 98, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);
    //MPI_Wait(&request, &status);
  ...
  for (int j=0;j<10000;j++){
    if (world_rank == 0) {   
      MPI_PreCtr_Send(c_send_buf, 872, MPI_CHAR, 1, 0, MPI_COMM_WORLD);
      //MPI_Wait(&request, &status);
    }else if (world_rank == 1) {
      MPI_PreCtr_Recv(c_recv_buf, 872, MPI_CHAR, 0, 0, MPI_COMM_WORLD, &status);
  ...
  if (world_rank == 0) {   
      MPI_PreCtr_Send(c_send_buf, 5120, MPI_CHAR, 1, 0, MPI_COMM_WORLD);
      //MPI_Wait(&request, &status);
    }else if (world_rank == 1) {
      MPI_PreCtr_Recv(c_recv_buf, 5120, MPI_CHAR, 0, 0, MPI_COMM_WORLD, &status);
