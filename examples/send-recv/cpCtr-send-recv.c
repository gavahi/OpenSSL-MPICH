...
int main(int argc, char** argv) {
  ...
  if (world_rank == 0) {   
    MPI_CpCtr_Send(send_buf, 1600, MPI_INT, 1, 0, MPI_COMM_WORLD);
    //MPI_Wait(&request, &status);
  } else if (world_rank == 1) {
    MPI_CTR_Recv(recv_buf, 1600, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
    //MPI_Wait(&request, &status);
  ...
  if (world_rank == 0) {   
    MPI_CpCtr_Send(d_send_buf, 10, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD);
    //MPI_Wait(&request, &status);
  } else if (world_rank == 1) {
    MPI_CTR_Recv(d_recv_buf, 10, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);
    //MPI_Wait(&request, &status);
  ...
  for (int j=0;j<10000;j++){
    if (world_rank == 0) {   
       MPI_CpCtr_Send(c_send_buf, 7872, MPI_CHAR, 1, 0, MPI_COMM_WORLD);
      //MPI_Wait(&request, &status);
    }else if (world_rank == 1) {
      MPI_CTR_Recv(c_recv_buf, 7872, MPI_CHAR, 0, 0, MPI_COMM_WORLD, &status);
  ...
  if (world_rank == 0) {   
      MPI_CpCtr_Send(c_send_buf, 10, MPI_CHAR, 1, 0, MPI_COMM_WORLD);
      //MPI_Wait(&request, &status);
    }else if (world_rank == 1) {
      MPI_CTR_Recv(c_recv_buf, 10, MPI_CHAR, 0, 0, MPI_COMM_WORLD, &status);
