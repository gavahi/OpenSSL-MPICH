                  ...
                  void *recvbuf, int recvcount, MPI_Datatype recvtype,  MPI_Comm comm)
{
	...
	EVP_EncryptUpdate(ctx_enc, ciphertext_send, &outlen_enc, sendbuf, blocktype_send);
	EVP_EncryptFinal_ex(ctx_enc, ciphertext_send+outlen_enc, &outlen_enc);
	EVP_CIPHER_CTX_ctrl(ctx_enc, EVP_CTRL_AEAD_GET_TAG, 16, ciphertext_send+blocktype_send);
		
	MPI_Allgather(ciphertext_send, blocktype_send+16, MPI_CHAR, ciphertext_recv,blocktype_recv+16, MPI_CHAR, comm);
	...
	for ( i=0; i<world_size ; i++ ){
		...
		EVP_DecryptUpdate(ctx_dec, recvbuf+i*blocktype_recv, &outlen_dec, ciphertext_recv+i*(blocktype_recv+16), blocktype_recv);		
		EVP_CIPHER_CTX_ctrl(ctx_dec, EVP_CTRL_AEAD_SET_TAG, 16,ciphertext_recv+i*(blocktype_recv+16)+blocktype_recv);		
		if (!(EVP_DecryptFinal_ex(ctx_dec, recvbuf+i*blocktype_recv, &outlen_dec) > 0)) printf("Tag Verify Failed!\n");
	
	}
	
	//my_print(recvbuf,sendcount * world_size ,"This Recv_Plain_Text");
