...
void my_print_Reduce(void *print_buffer, int count,char s[])	//keep square brackets in signature
{ 
	printf("\n%s=\n",s);fflush(stdout);
...
int MPI_SEC_Reduce(const void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, MPI_Op op, int root, MPI_Comm comm)
{
	...
	static unsigned char gcm_key[32] = "01234567890123456789012345678901";
	static unsigned char gcm_iv[16] = "012345678901";
	static unsigned int ADDITIONAL_DATA_LEN = 16;
	static unsigned char gcm_aad[] = "0123456789012345";
	...
	if (rank != root) {
		...
		//my_print_Reduce(&sendbuf,count,"This Send_Plain_Text");
		...
		EVP_EncryptUpdate(ctx_enc, ciphertext, &outlen_enc, sendbuf, count);
		...
		EVP_CIPHER_CTX_ctrl(ctx_dec, EVP_CTRL_AEAD_SET_IVLEN, sizeof(gcm_iv), NULL);
		EVP_DecryptInit_ex(ctx_dec, NULL, NULL, gcm_key, gcm_iv);
		//EVP_DecryptUpdate(ctx_dec, NULL, &outlen_dec, gcm_aad, sizeof(gcm_aad))
			
		MPI_Reduce(sendbuf, ciphertext, count+16, datatype, op,root,comm);
		
		EVP_DecryptUpdate(ctx_dec, recvbuf, &outlen_dec, ciphertext, count);	
		EVP_CIPHER_CTX_ctrl(ctx_dec, EVP_CTRL_AEAD_SET_TAG, 16,ciphertext+count);	
		if (!(EVP_DecryptFinal_ex(ctx_dec, recvbuf, &outlen_dec) > 0)) printf("Tag Verify Failed!\n");		
		
		//count = decrypt_Reduce(ciphertext, ciphertext_len, aad, strlen(aad), recv_tag, key, iv,&recvbuf);
		
		//my_print_Reduce(&recvbuf,count,"This Recv_Plain_Text =");	
	}
