                  ...
                  void *recvbuf, int recvcount, MPI_Datatype recvtype,  MPI_Comm comm)
{
	...
	for (int i=0; i<world_size ; i++ ){
		...
		dest = (unsigned long long) i*blocktype_send;
		src =(unsigned long long) i*(blocktype_send+16);
		openssl_enc_core(ciphertext_send,src,sendbuf,dest,blocktype_send);
		
	}
		
	MPI_Alltoall(ciphertext_send, (blocktype_send+16), MPI_CHAR, ciphertext_recv,blocktype_recv+16, MPI_CHAR, comm);
	...
	for (int i=0; i<world_size ; i++ ){
		...
		src =(unsigned int)i*(blocktype_recv+16);
		openssl_dec_core(ciphertext_recv,src,recvbuf,dest,blocktype_recv);			
	}
